...
const technologies = [
    ...
    { logo: "https://cdn.simpleicons.org/divi/6C2EB9", name: "Divi", level: "Expert" },
...
const MarqueeItem = ({ logo, name }) => (
    <div className="flex items-center gap-3 px-8 py-4 bg-white dark:bg-surface/50 border border-slate-200 dark:border-white/10 rounded-full hover:border-primary/50 dark:hover:border-primary/50 hover:bg-slate-50 dark:hover:bg-surface-elevated/80 transition-all cursor-default shadow-sm dark:shadow-none">
        <img src={logo} alt={name} className="w-6 h-6" />
        <span className="text-xl font-medium text-slate-700 dark:text-white whitespace-nowrap">{name}</span>
    </div>
);
    ...
    <div className="flex overflow-hidden relative w-full mask-linear-fade">
        ...
        >
            {[...items, ...items, ...items].map((tech, i) => (
                <MarqueeItem key={i} logo={tech.logo} name={tech.name} />
            ))}
        </motion.div>
    </div>
);
        ...
        <section id="stack" className="py-32 bg-transparent relative overflow-hidden">
            ...
            <div className="container mx-auto px-6 mb-20 relative z-10">
                <motion.h2
                    ...
                    viewport={{ once: true }}
                    className="text-3xl md:text-5xl font-bold text-center font-heading text-slate-900 dark:text-white"
                >
